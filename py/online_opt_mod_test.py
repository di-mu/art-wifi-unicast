...
def search_test(x,y,prrw,prrz, Dt, hw, hz):
    ...
    if not power_val:
        ...
        return row[(loc[idx][0])], col[(loc[idx][1])]#, feasible[power_val.index(min(power_val))], min(power_val)

#erroneous runtime error catching
np.seterr(all='ignore') 

for ii in range(0,10000):
    arg = (random.randrange(22), random.randrange(9), random.randrange(101), random.randrange(101),
        random.randrange(901) + 100, random.randrange(100) + 750, random.randrange(50) + 200)
    start = time.clock()
    search_test(row[arg[0]], col[arg[1]], float(arg[2])/100, float(arg[3])/100, arg[4], arg[5], arg[6])
    ms = int((time.clock() - start) * 1000)
    print str(ms) + " ms"
